...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        for (Class<?> clazz = pageClass; actionMethod == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                actionMethod = clazz.getDeclaredMethod(route.getAction(), HttpServletRequest.class, Map.class);
        ...
        try {
            ...
            if (actionMethod.getName().equals("action")) {
                Method beforeMethod = null;
                Method afterMethod = null;

                for (Class<?> clazz = pageClass; beforeMethod == null && clazz != null; clazz = clazz.getSuperclass()) {
                    try {
                        beforeMethod = clazz.getDeclaredMethod("before", HttpServletRequest.class, Map.class);
                    } catch (NoSuchMethodException ignored) {
                        // No operations.
                    }
                }

                for (Class<?> clazz = pageClass; afterMethod == null && clazz != null; clazz = clazz.getSuperclass()) {
                    try {
                        afterMethod = clazz.getDeclaredMethod("before", HttpServletRequest.class, Map.class);
                    } catch (NoSuchMethodException ignored) {
                        // No operations.
                    }
                }

                Objects.requireNonNull(beforeMethod).invoke(page, request, view);
                actionMethod.invoke(page, request, view);
                Objects.requireNonNull(afterMethod).invoke(page, request, view);
            } else {
                actionMethod.invoke(page, request, view);
            }
